...
window.onload = function() {
    ...
    alert(div.childNodes.length); //IE6-8弹出3，其他4
    alert(div.firstChild.nodeType) //IE6-6弹出1，其他3
}

window.onload = function() { //请在IE6-8下测试
    var div = document.createElement("div");
...
var tagHooks = {
	...
	tr: [2, "<table><tbody>"],
	td: [3, "<table><tbody><tr>"],
	//IE6-8在用innerHTML生成节点时，不能直接创建script,link,meta,style与HTML5的新标签
...
$.fn.parseHTML = function(html, doc) {
	...
	if ($.support.noscope) { //fix IE，在link style script等no-scope元素前打个补丁
		html = html.replace(rcreate, "<br class=fix_noscope>$1");丁
	}
	...
	$.fixParseHTML(wrapper, html); //对于IE6-8进行处理
...
window.onload = function() {
	...
	alert(clone.expando.key )//IE6-8：2;其他：undefined
}
...
window.onload = function() {
	...
	alert(div.parentNode); //IE6-8 object;其他 null
	if (div.parentNode) {
...
if ("1"[0]) {//IE6-7这里返回undefined,于是跑到第二个分支
    var iframe = document.createElement("iframe");
    iframe.name = name;
} else {
    ...
    <head>
        ...
        <script>
            if (window.postMessage) {
                window.parent.postMessage("测试HTML5的postMessage!", "*")
...
window.onload = function() {
alert(document.body.getElementsByTagName("*").length) //IE6-8为1
alert(document.body.getElementsByTagName("param").length) //3
}


<object type="application/x-shockwave-flash" data="test.swf" width="128" height="128" ><param name="movie" value="test.swf" /></object>

<embed type="application/x-shockwave-flash" src="test.swf" width="128" height="128" ></embed>
