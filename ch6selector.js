...
window.onload = function() {
    ...
    log(els);
    els = shuffle(els); //洗牌（模似选择器引擎最初得到的结果集的情况）
    log(els);
...
switch (match[1]) {
	...
	case ":":
		...
		//Icarus.pseudoHooks里面放置我们所有能处理伪类
...
//备份,因为context会被改写,如果出现并联选择器,没备分会找不到北
...
//将会得到parts数组为： ["body","div",">", "div:not(.aaa)"]
...
Sizzle.filter = function(expr, set, inplace, not) {
	...
	//expr: 为选择符
	//set:  为元素数组
	//inplace:为undefined, null时进入生成种子集模式；为true时进入映射集模式
	//not:  一个布尔值,来源自取反选择器
	var match, anyFound,
